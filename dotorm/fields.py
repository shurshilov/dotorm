...
class Field[FieldType]:
    ...
    required - Analog null option but reversed, has priority

    index - real index in database create or not?
...
class Binary(Field[bytes]):
    ...
    # TODO: postgres also have VARBINARY
    sql_type = "BYTEA"
    indexable = False
...
class AttachmentMany2one[T: "DotModel"](Field[T]):
    """
    Many2one attachment field.
    """

    # TODO:
    # должен быть тру для чтения аттачментов из базы без содержимого
    # должен быть фалс для того чтобы избежать обновления стандартным путем
    # так как каждый раз надо создавать запись
    # store = True
    field_type = Type
    sql_type = "INTEGER"
    relation = True
